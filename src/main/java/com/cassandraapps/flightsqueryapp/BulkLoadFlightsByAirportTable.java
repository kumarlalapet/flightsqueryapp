...
public class BulkLoadFlightsByAirportTable {
	...
	public static final String SCHEMA = String.format("CREATE TABLE %s.%s ("
			+ "\"ID\" int ," + "\"ORIGIN_AIRPORT_ID\" int, "
			+ "\"ORIGIN_AIRPORT\" varchar," + "\"FL_DATE_TIME\" timestamp,"
			+ "\"AIRLINE_ID\" int, " + "\"CARRIER\" varchar, "
			+ "\"FL_NUM\" int, "
			+ "\"DEST\" varchar, "
			+ "PRIMARY KEY (\"ORIGIN_AIRPORT\", \"FL_DATE_TIME\", \"ID\") )", KEYSPACE, TABLE);
	...
	public static final String INSERT_STMT = String.format(
			"INSERT INTO %s.%s ("
					+ "\"ID\", \"ORIGIN_AIRPORT_ID\", \"ORIGIN_AIRPORT\", \"FL_DATE_TIME\", \"AIRLINE_ID\", \"CARRIER\", \"FL_NUM\", "
					+ "\"DEST\" "
					+ ") VALUES ("
					+ "?, ?, ?, ?, ?, ?, ?, ?" + ")",
			KEYSPACE, TABLE);
