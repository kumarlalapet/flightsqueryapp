...
public class BulkLoadFlightsTable {
	...
	public static final String SCHEMA = String.format("CREATE TABLE %s.%s ("
			...
			+ "\"DAY_OF_MONTH\" int," + "\"FL_DATE\" timestamp,"
			+ "\"AIRLINE_ID\" int, " + "\"CARRIER\" varchar, "
			+ "\"FL_NUM\" int, " + "\"ORIGIN_AIRPORT_ID\" int,"
			+ "\"ORIGIN\" varchar, " + "\"ORIGIN_CITY_NAME\" varchar,"
			+ "\"ORIGIN_STATE_ABR\" varchar, " + "\"DEST\" varchar, "
			+ "\"DEST_CITY_NAME\" varchar," + "\"DEST_STATE_ABR\" varchar,"
			+ "\"DEP_TIME\" timestamp," + "\"ARR_TIME\" timestamp,"
			+ "\"ACTUAL_ELAPSED_TIME_IN_MINS\" int,"
			+ "\"AIR_TIME_IN_MINS\" int," + "\"DISTANCE\" int" + ")", KEYSPACE,
			TABLE);
	...
	public static final String INSERT_STMT = String.format(
			"INSERT INTO %s.%s ("
					+ "\"ID\", \"YEAR\", \"DAY_OF_MONTH\", \"FL_DATE\", \"AIRLINE_ID\", \"CARRIER\", \"FL_NUM\", \"ORIGIN_AIRPORT_ID\", \"ORIGIN\", \"ORIGIN_CITY_NAME\", " 
					+ "\"ORIGIN_STATE_ABR\", \"DEST\", \"DEST_CITY_NAME\", \"DEST_STATE_ABR\", \"DEP_TIME\", \"ARR_TIME\", \"ACTUAL_ELAPSED_TIME_IN_MINS\", \"AIR_TIME_IN_MINS\", \"DISTANCE\""
					+ ") VALUES ("
					+ "?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?,?,?,?" + ")",
			KEYSPACE, TABLE);
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(
			...
			while ((line = csvReader.read()) != null) {
				...
				writer.addRow(Integer.parseInt(id), 
						...
						fl_date_dt, 
						Integer.parseInt(airline_id), 
						carrier, 
						Integer.parseInt(fl_num), 
						...
						dep_time_dt, 
						arr_time_dt, 
						Integer.parseInt(actual_elapsed_time), 
						Integer.parseInt(air_time), 
